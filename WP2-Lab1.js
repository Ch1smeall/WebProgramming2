...
console.log(`This house at ${listings1.address} is owned by ${listings1.currentOwner}`);
 ...
 * and returns a a string with the dollar amount as realtor dees using the following conditions, 
...
function realtorFees(listing){
  ...
  let realtorFee = price <= 450000 ? price * 0.025 : price * 0.02;

  realtorFee = parseInt(realtorFee);
...
function sortAscendingly(listings){
  let listingAscendingly = [...listings];

  for(let ii = 0; ii < listingAscendingly.length - 1; ii++){
    for(let jj = 0; jj < listingAscendingly.length - 1 - jj; jj++){
      if(realtorFees(listingAscendingly[jj]) > realtorFees(listingAscendingly[jj + 1])){
        let temp = listingAscendingly[jj];
        listingAscendingly[jj] = listingAscendingly[jj + 1];
        listingAscendingly[jj + 1] = temp;
      }
    }
  }
  return listingAscendingly;
}
...
let newBuiltListings = listings.filter(listing =>{
  let builtYear = listing.propertySummary.builtIn;

  builtYear = parseInt(builtYear);

  return builtYear >= 1990;
});
